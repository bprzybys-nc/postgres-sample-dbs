# Create complex business logic for employees database (Logic-Heavy scenario)
employee_payroll_config = """# src/business/employee_payroll_system.py
...
class EmployeePayrollSystem:
        ...
        try:
            ...
            if row:
                return Employee(
                    ...
                    current_salary=Decimal(str(row['current_salary'])) if row['current_salary'] else None,
    ...
    async def calculate_payroll(self, emp_no: int, pay_period_start: date, pay_period_end: date, 
                              overtime_hours: Decimal = Decimal('0')) -> PayrollCalculation:
        \"\"\"
        ...
        if not employee or not employee.current_salary:
            raise ValueError(f"Cannot calculate payroll for employee {emp_no}: missing salary data")
        ...
        if overtime_hours > 0:
            hourly_rate = employee.current_salary / Decimal('2080')  # 40 hours * 52 weeks
    ...
    async def process_department_payroll(self, department: str, pay_period_start: date, 
                                       pay_period_end: date) -> List[PayrollCalculation]:
        \"\"\"
        CRITICAL OPERATION: Process entire department payroll
        
        Business Impact: Processes millions in payroll per department
        Compliance: Must maintain audit trail and SOX controls
        \"\"\"
        ...
        try:
            ...
            payroll_results = []
            total_department_payroll = Decimal('0')
            
            for row in rows:
                emp_no = row['emp_no']
                try:
                    calculation = await self.calculate_payroll(emp_no, pay_period_start, pay_period_end)
                    payroll_results.append(calculation)
                    total_department_payroll += calculation.gross_pay
                    
                except Exception as e:
                    logger.error(f"Failed to calculate payroll for employee {emp_no}: {e}")
                    # In production, this would trigger compliance alerts
            
            logger.info(f"Processed {len(payroll_results)} employees in {department}")
            logger.info(f"Total department gross payroll: ${total_department_payroll:,.2f}")
            
            return payroll_results
            
        except Exception as e:
            logger.error(f"Department payroll processing failed for {department}: {e}")
            raise
    ...
    async def generate_compliance_report(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        try:
            ...
            total_annual_payroll = sum(row['total_salary'] for row in salary_dist_results)
            ...
            compliance_report = {
                ...
                "employee_metrics": {
                    ...
                    "average_salary": float(total_annual_payroll / total_employees) if total_employees > 0 else 0
                },
                "salary_distribution": [
                    {
                ...
                "compliance_status": {
                    ...
                    "requires_cfo_approval": total_annual_payroll > 10000000  # $10M threshold
                }
            }
    ...
    async def detect_payroll_anomalies(self) -> List[ComplianceAlert]:
        \"\"\"
        CRITICAL CONTROL: Detect payroll anomalies for compliance monitoring
        
        Triggers for:
        - Unusual salary changes (>25% increase)
        ...
        # Detect large salary increases
        salary_increase_query = \"\"\"
        ...
        SELECT emp_no, salary, prev_salary, 
               (salary - prev_salary) / prev_salary * 100 as pct_increase
        FROM salary_changes
        WHERE prev_salary IS NOT NULL 
        AND (salary - prev_salary) / prev_salary > 0.25
        \"\"\"
        
        try:
            conn = await asyncpg.connect(self.connection_string)
            anomaly_results = await conn.fetch(salary_increase_query)
            await conn.close()
            
            for row in anomaly_results:
                alert = ComplianceAlert(
                    alert_id=f"PAY_ANOM_{row['emp_no']}_{datetime.utcnow().strftime('%Y%m%d')}",
                    emp_no=row['emp_no'],
                    alert_type="SALARY_ANOMALY",
                    severity="HIGH",
                    description=f"Employee {row['emp_no']} has salary increase of {row['pct_increase']:.1f}% - requires review",
...
# Service factory for payroll system
class PayrollSystemFactory:
    \"\"\"Factory for creating payroll system instances\"\"\"
    
    @staticmethod
    def create_production_system(connection_string: str) -> EmployeePayrollSystem:
        \"\"\"Create production payroll system with full compliance\"\"\"
        return EmployeePayrollSystem(connection_string)
    
    @staticmethod
    def create_test_system(connection_string: str) -> EmployeePayrollSystem:
        \"\"\"Create test payroll system (limited functionality)\"\"\"
        system = EmployeePayrollSystem(connection_string)
        # In test mode, would disable actual payment processing
        return system
...
# Save the employee payroll system file
with open("employee_payroll_system.py", "w") as f:
    f.write(employee_payroll_config)

print("✅ Created complex business logic for employees database")
print("File: src/business/employee_payroll_system.py")
print("Contains: Critical payroll operations, SOX compliance, multi-million dollar processing")
print("Features: Tax calculations, compliance reporting, anomaly detection")